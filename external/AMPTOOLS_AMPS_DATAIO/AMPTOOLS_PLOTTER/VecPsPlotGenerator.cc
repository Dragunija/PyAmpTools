...
#include "AMPTOOLS_PLOTTER/VecPsPlotGenerator.h"
...
void VecPsPlotGenerator::createHistograms( ) {
  cout << " calls to bookHistogram go here" << endl;
   ...
   bookHistogram( kVecPsMass, new Histogram1D( 25, 1.5, 2.5, "MVecPs", "Invariant Mass of Vec+Ps [GeV]") );
   bookHistogram( kCosTheta, new Histogram1D( 25, -1., 1., "CosTheta", "cos#theta" ) );
   bookHistogram( kPhi, new Histogram1D( 25, -1*PI, PI, "Phi", "#phi [rad.]" ) );
   bookHistogram( kCosThetaH, new Histogram1D( 25, -1., 1., "CosTheta_H", "cos#theta_H" ) );
   bookHistogram( kPhiH, new Histogram1D( 25, -1*PI, PI, "Phi_H", "#phi_H [rad.]" ) );
   bookHistogram( kProd_Ang, new Histogram1D( 25, -1*PI, PI, "Prod_Ang", "Prod_Ang [rad.]" ) );
   //bookHistogram( kCosThetaPhi, new Histogram2D(25, -1, 1, 25, -1*PI, PI, "CosTheta_Phi", "CosTheta_Phi") );
   //bookHistogram( kCosThetaProd, new Histogram2D(25, -1, 1, 25, -1*PI, PI, "CosTheta_Prod", "CosTheta_Prod" ) );
   //bookHistogram( kPhiProd, new Histogram2D(25, -1*PI, PI, 25, -1*PI, PI, "Phi_Prod", "Phi_Prod" ) );
   //bookHistogram( kCosThetaHPhiH, new Histogram2D( 25, -1, 1, 25, -1*PI, PI, "CosThetaH_PhiH", "CosThetaH_PhiH" ) );
   bookHistogram( kt, new Histogram1D( 25, 0, 2.0 , "t", "-t" ) );
   bookHistogram( kRecoilMass, new Histogram1D( 25, 0.9, 1.9 , "MRecoil", "Invariant Mass of Recoil [GeV]" ) );
   bookHistogram( kProtonPsMass, new Histogram1D(25, 0.9, 2.9, "MProtonPs", "Invariant Mass of proton and bachelor Ps [GeV]" ) );
   bookHistogram( kRecoilPsMass, new Histogram1D(25, 0.9, 2.9, "MRecoilPs", "Invariant Mass of recoil and bachelor Ps [GeV]" ) );
   bookHistogram( kLambda, new Histogram1D(25, 0.0, 1.1, "Lambda", "#lambda_{#omega}" ) );
   bookHistogram( kDalitz, new Histogram2D(25, -2., 2.,25, -2., 2., "Dalitz", "Dalitz XY" ) );
}
