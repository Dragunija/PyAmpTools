...
def PlotGen(
    ...
    if plotter == "etapi":
        ...
        plotGen = VecPsPlotGenerator(results, plotGenHists)
    ...
    for waveset in wavesets:
        ...
        for isum in range(len(sums) + 1):  # loop over sums, last one combines them all
            ...
            if plotter == "etapi":
                ...
                print(f" >> PlotGenerator has {VecPsPlotGenerator.kNumTypes} data source. Generating up to {VecPsPlotGenerator.kNumHists} histograms each")
                for iplot in range(VecPsPlotGenerator.kNumTypes):
                    if isum < sums.size() and iplot == VecPsPlotGenerator.kData:
                        continue
                    for ivar in range(VecPsPlotGenerator.kNumHists):
                        for ireact, reactionName in enumerate(reactionList):
                            histName = reactionName + "_"
                            if not plotAllVars and ivar > 3:
                                continue
                            if not plotGenHists and iplot == VecPsPlotGenerator.kGenMC:
                                continue
                            if ivar == VecPsPlotGenerator.kVecPsMass:
                                histName += "MVecPs"
                            elif ivar == VecPsPlotGenerator.kCosTheta:
                                histName += "CosTheta"
                            elif ivar == VecPsPlotGenerator.kPhi:
                                histName += "#phi"
                            elif ivar == VecPsPlotGenerator.kCosThetaH:
                                histName += "CosTheta_H"
                            elif ivar == VecPsPlotGenerator.kPhiH:
                                histName += "#Phi_H"
                            elif ivar == VecPsPlotGenerator.kProd_Ang:
                                histName += "Prod_Ang"
                            elif ivar == VecPsPlotGenerator.kt:
                                histName += "t"
                            elif ivar == VecPsPlotGenerator.kRecoilMass:
                                histName += "MRecoil"
                            elif ivar == VecPsPlotGenerator.kProtonPsMass:
                                histname += "MProtonPs"
                            elif ivar == VecPsPlotGenerator.kRecoilPsMass:
                                histName += "MRecoilPs"
                            elif ivar == VecPsPlotGenerator.kLambda:
                                histName += "Lambda"
                            elif ivar == VecPsPlotGenerator.kDalitz:
                                histName += "Dalitz"
                            else:
                                print(">> WARNING: Unknown variable. Exiting...")
                                exit(1)

                            if iplot == VecPsPlotGenerator.kData:
                                histName += "dat"
                            if iplot == VecPsPlotGenerator.kBkgnd:
                                histName += "bkg"
                            if iplot == VecPsPlotGenerator.kAccMC:
                                histName += "acc"
                            if iplot == VecPsPlotGenerator.kGenMC:
                                histName += "gen"
                        ...
                        thist = hist.toRoot()
                        if thist.Integral() != 0 and save is True:  # only write if there are entries
                            print(f" >> Writing histogram {histName} to file")
                            thist.SetName(histName)
                            plotfile.cd()
                            thist.Write()
                        elif thist.Integral != 0 and save is False:
                            thist.SetName(histName)
                            hists[name] = thist
                        else:
...
def _cli_plotgen():
    ...
    parser.add_argument("-plotter", type=str, default="vec_ps", help="Select plotter class")
    ...
    PlotGen(results, ofile, ampString, keepAllAmps, plotAllVars, plotGenHists, doAccCorr, plotter)
